...
describe('Bridge', function () {
    ...
    before(async () => {
        ...
        const bridgeAddress = fs.readFileSync(__dirname + "/../build/contract_address").toString().split(' ')[1]
    ...
    it("get consumed actions", async () => {
        const result = await bridge.methods.getCosumedActions()
        console.log(result)
        // assert.ok(isInitialized.eq(new BN(1)))
    })
});